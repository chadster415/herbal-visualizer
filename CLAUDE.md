...
## Guides
- **[Parsing class notes](docs/parsing-class-notes.md)** — **Use this first when the user says "add class NN".** Covers finding the class files in the Obsidian vault, extracting snippets, keywords, ailment synonyms, herb pairs, and quiz questions into migrations. Includes the full checklist and SQL patterns. **If a medicinal herb in the notes is not yet in the DB, add it first using the adding-herb-data-from-books playbook — do not skip actual herbs.** Only skip non-herb items (foods, preparations, vague category words like "a lymphatic").
